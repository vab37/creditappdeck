...
```{r, echo=TRUE, eval=FALSE, tidy=TRUE}
predCredit <- function(inpAge, inpForeignWorker, inpProperty.RealEstate, inpHousing.Own, inpCreditHistory.Critical){
pred = predict(mod_fit, newdata=data.frame(Age = inpAge, ForeignWorker=inpForeignWorker, Property.RealEstate=inpProperty.RealEstate, Housing.Own=inpHousing.Own, CreditHistory.Critical=inpCreditHistory.Critical))
as.character(pred)
}
```
...
The server interacts with the prediction model and UI to provide user inputs to the prediction function and the final prediction back to the UI  
...
server <- function(input, output) {
  output$prediction <- renderPrint({
    predCredit(input$iAge,
        as.numeric(as.character(input$iForeignWorker)),
        as.numeric(as.character(input$iProperty.RealEstate)),
        as.numeric(as.character(input$iHousing.Own)),
